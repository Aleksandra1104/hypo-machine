...
public class HypoMachine3 {
    ...
    public static void main(String[] args) {
        ...
        while(!shutdown) {
            ...
            checkAndProcessInterrupt(scanner, fileWriter, consoleWriter);
            ...
            printBoth(fileWriter, consoleWriter, "Selected process to be given CPU:");
            ...
            switch((int)status) {
                ...
                case 8:
                    ...
                    break;
                case 9:
                    statusDecoder = "Output operation System Call occurred";
                    break;
                default:
                    statusDecoder = "Unknown Error occurred.";
                    break;
            }
    ...
    private static void executeArithmetic( char operator, long Op1Mode, long Op1GPR, long Op2Mode, long Op2GPR ) {
        ...
        if(Op1Mode == 1) {
            GPRs[(int)Op1GPR] = result; // If Op1Mode == 1, stores the result value into GPR
        } else if (Op1Mode == 6) {
    ...
    private static long systemCall(long systemCallId, PrintWriter fileOut, PrintWriter consoleOut) {
        ...
        switch((int)systemCallId) {
            ...
            case 4:     // Dynamic memory allocation: Allocate user free memory system call
                status = MemAllocSystemCall(fileOut, consoleOut);
                break;
            case 5:     // Free dynamically allocated user memory system call
                status = FreeMemSystemCall(fileOut, consoleOut);
                break;
            case 6:     // Message Sent 
    ...
    private static String freeOSMemory(long ptr, long size) {
        ...
        if(ptr < OSFreeList) {      // if ptr < address pointed by UserFreeList, insert the block in the beginning of the list
            ...
            while(ptr > memory[(int)currentAddr] && memory[(int)currentAddr] != EndOfList){
                currentAddr = memory[(int)currentAddr];
            }
            
            if(ptr + size == memory[(int)currentAddr]) {    // merge if the right  neighbor blocks intersect
            ...
            } else if(currentAddr + memory[(int)currentAddr + 1] == ptr){   // merge if the left neighbor blocks intersect
                memory[(int)currentAddr + 1] = memory[(int)currentAddr + 1] + size;
            }  else {
        ...
        if(size < 1 || (ptr + size) >= 6999 ) {
            ...
            if(ptr < UserFreeList) {    // if ptr < address pointed by UserFreeList, insert the block in the beginning of the list
                ...
                if(ptr + size == memory[(int)currentAddr]) {    // merge if the right neighbor blocks intersect
                    ...
                    memory[(int)ptr + 1] = size + memory[(int)memory[(int)currentAddr] + 1];
                    memory[(int)currentAddr] = ptr;
                } else if(currentAddr + memory[(int)currentAddr + 1] == ptr){
                    memory[(int)currentAddr + 1] = memory[(int)currentAddr + 1] + size;
                } else {
                    memory[(int)ptr] = memory[(int)currentAddr];
                    memory[(int)ptr + 1] = size;
    ...
    // Return: currentPCBptr
    // Written by Maxwell Whelan
    private static long searchPCBfromRQ(long pid) {
    ...
    // Error code: -1
    // Written by Sasha Basova
    private static long terminateProcess(long PCBptr, PrintWriter fileWriter, PrintWriter consoleWriter) {
        long errorCode = -1;
        long successCode = 1;
    ...
    private static void checkAndProcessInterrupt(Scanner scanner, PrintWriter fileOut, PrintWriter consoleOut) {
        ...
        String interruptId = scanner.nextLine();
        int interruptID = Integer.parseInt(interruptId);
        printBoth(fileOut, consoleOut, "You chose " + interruptID);
        ...
        switch(interruptID) {
            case 0:     // no interrupt
                break;
            case 1:     // run program
                ISRrunProgramInterrupt(scanner, fileOut, consoleOut);
                break;
            ...
            case 4:     // output operation completion interrupt
                ISRoutputCompletionInterrupt(scanner, fileOut, consoleOut);
                break;
            default:
    ...
    private static void ISRrunProgramInterrupt(Scanner scanner, PrintWriter fileOut, PrintWriter consoleOut) {
        ...
        printBoth(fileOut, consoleOut, "You entered: " + filename);
    ...
    private static void ISRshutdownSystem(PrintWriter fileOut, PrintWriter consoleOut) {
        ...
        while(ptr != EndOfList) {
            RQ = memory[(int)ptr];
            terminateProcess(ptr, fileOut, consoleOut);
            ptr = RQ;
        }
    ...
    private static void ISRinputCompletionInterrupt(Scanner scanner, PrintWriter fileOut, PrintWriter consoleOut) {
        ...
        if(PCBptr > 0) {
            ...
            printBoth(fileOut, consoleOut, "Enter one character:  ");
            ...
            if(PCBptr > 0) {
                ...
                printBoth(fileOut, consoleOut, "PCB with entered PID is found in Ready Queue.");
                printBoth(fileOut, consoleOut, "Enter one character:  ");
